  ...
  return (
    <div style={{
      ...
      <div style={{ textAlign: 'right', flexShrink: 0 }}>
        ...
        }}>${pv?.toLocaleString('en', { maximumFractionDigits: 0 })}</div>
...
export default function ArenaScreen({ selectedAgents, startingCash = 10000, onExit }) {
  ...
  if (!gameState) {
    return (
      <div style={{ width:'100vw', height:'100vh', display:'flex', alignItems:'center', justifyContent:'center', background:'var(--bg0)', flexDirection:'column', gap:16 }}>
        ...
        <div style={{ fontFamily:'var(--fm)', color:'var(--t3)', fontSize:10 }}>
          {connected ? 'Server connected — waiting for game state' : 'Establishing WebSocket…'}
        </div>
        {!connected && (
          <div style={{ marginTop:8, padding:'8px 18px', background:'rgba(255,215,0,.08)', border:'1px solid rgba(255,215,0,.3)', borderRadius:8, fontFamily:'var(--fm)', fontSize:10, color:'var(--gold)', textAlign:'center' }}>
  ...
  return (
    <div style={{ width:'100vw', minHeight:'100vh', display:'flex', flexDirection:'column', background:'var(--bg0)', overflowX:'hidden', overflowY:'auto' }}>
      ...
      <div style={{
        ...
        {/* LEFT — stock list */}
        <div style={{ height: 'calc(100vh - 58px)', position: 'sticky', top: 58 }}>
          <StockPanel
        ...
        <div style={{ display: 'flex', flexDirection: 'column', gap: 8 }}>
          ...
          />
          <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: 8, height: 280 }}>
        ...
        <div style={{ display:'flex', flexDirection:'column', gap:8, position:'sticky', top:58, height:'calc(100vh - 58px)', overflow:'auto' }} className="scroll">
          <LeaderboardPanel leaderboard={leaderboard} startingCash={startingCash} />

          <div className="glass" style={{ borderRadius:10, border:'1px solid rgba(0,255,136,.2)', overflow:'hidden', flex:1, minHeight:200, display:'flex', flexDirection:'column' }}>
            ...
            <div className="scroll" style={{ flex:1 }}>
              <div style={{ padding:'7px 13px', display:'flex', justifyContent:'space-between', borderBottom:'1px solid rgba(0,212,255,.07)' }}>
                ...
                <span style={{ fontFamily:'var(--fm)', fontSize:10, color:'var(--c)' }}>${(player.cash||0).toFixed(2)}</span>
              </div>
                  ...
                  return (
                    <div key={tk} onClick={() => setSelected(tk)} style={{ padding:'7px 13px', borderBottom:'1px solid rgba(0,212,255,.04)', cursor:'pointer' }}>
                      <div style={{ display:'flex', justifyContent:'space-between', marginBottom:2 }}>
                        ...
                        <span style={{ fontFamily:'var(--fm)', fontSize:11, color:'var(--t1)' }}>${(pos.shares*price).toFixed(0)}</span>
                      </div>
